...
| **Full inspection** | Spot-check, walk, walkaround | The act/record of checking a vehicle. Changed from "Spot-check" on 2026-08-24 to match the shipped RN build. Walk-around stays for the physical procedure. |
| **Defect** | Fault, issue, fail | A problem found during a spot-check. |
| **Driver** | Keyholder | Who currently holds the vehicle (confirmed at check start). Changed from "Keyholder" on 2026-08-24 to match the shipped RN build. |
...
## Procedure terms
- **Walk-around** (always hyphenated) — the physical inspection procedure and the checklist's name ("Vehicle walk-around", "walk-around photos"). The *record* it produces is a **full inspection**; never call the act itself "a walk".

| Use | Notes |
|---|---|
| **Inspector** | Does full inspections. |
