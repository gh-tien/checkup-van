...
Status: 23 of 23 complete; phase 4 complete. **v30 is live in production; v31 is committed to source (not yet deployed), verified in preview 2026-08-20.** v31 = setup-gate copy trim (dropped the explanatory paragraph; big inked header + big inked version stamp). v30 = first-run setup-gate refinement (whole-screen centred gate, top bar + tabs dropped on `setup`, admin modal moved into `viewSetup()`, build stamp at the foot). Prior: v28 durable storage + header build stamp; v29 hidden admin role + first-run setup gate. One deploy (v31) outstanding.   | batch size: 1 (session 0cc2950e dropped repeatedly with ECONNRESET)
